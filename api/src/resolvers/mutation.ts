...
const isValidSign = (username: string, pass: string) => {
  return !(
    (!/\w/.test(username) && !/\p{Script=Cyrillic}+/u.test(username)) ||
    pass.length < 4 ||
    username.length > 200 ||
    pass.length > 200
  );
};
const isValidPost = (title: string, content: string) => {
  return !(title.length > 200 || content.length > 100_000);
};
...
const mutation: MutationResolvers = {
  ...
  newPost: async (parent, { title, content, file }, { models, user }) => {
    ...
    if (!isValidPost(title, content)) throw new Error("Ошибка данных");
